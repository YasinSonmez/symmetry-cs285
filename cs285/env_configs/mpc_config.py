...
def mpc_config(
    ...
    if env_name == "obstacles-cs285-v0":
        ep_len = 100
    if ep_len is None:
        ep_len = 200 # Some default value
    

    def make_dynamics_model(in_ob_dim: int, ac_dim: int, out_ob_dim: int) -> nn.Module:
        return ptu.build_mlp(
            input_size=in_ob_dim + ac_dim,
            output_size=out_ob_dim,
            n_layers=num_layers,
            size=hidden_size,
        )
