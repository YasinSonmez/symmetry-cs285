...
def _legacy_make_env(
    ...
    if "dmcontrol___" in cfg.overrides.env:
        ...
        elif cfg.overrides.env == "custom___Reacher":
            env = mbrl.env.reacher.CustomReacherEnv()
            observation_edit = mbrl.env.reacher.CustomReacherEnv.observation_edit
            env = gym.wrappers.TransformObservation(env, observation_edit)
        ...
        elif cfg.overrides.env == "custom___Car":
            env = mbrl.env.car.CarEnv()
            term_fn = mbrl.env.termination_fns.no_termination
            reward_fn = mbrl.env.reward_fns.car
            env = gym.wrappers.TimeLimit(
                env, max_episode_steps=cfg.overrides.get("trial_length", 200)
            )
        else:
